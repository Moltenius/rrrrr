...
let PF: {
  data: { [key: string]: JSONCompliantObject };
  save: () => void;
  write: (key: string, value: JSONCompliantObject) => void;
  read: (key: string) => JSONCompliantObject;
} = {
  data: {},
  save: function () {
...
let clamp = (min: number, max: number, target: number) =>
  Math.min(Math.max(target, min), max);

let make_glot_post: (data: string, postName?: string) => Promise<string> = (
  data: string,
  postName?: string
) => {
  let TargetPostName: string =
    postName || `${new Date().toUTCString()} Log Export - RoCtrl`;
  return new Promise((resolve, reject) => {
    axios
      .post("https://glot.io/api/snippets", {
        language: "lua",
        title: TargetPostName,
        public: true,
        files: [{ name: "export.txt", content: data }],
      })
      .then((data) => {
...
let _TimeFormat = (seconds: number) => {
  ...
  let secs = seconds % 60;
  return `${mins || ""}${mins ? ` minute${mins == 1 ? "" : "s"}` : ""}${
    secs && mins ? " and " : ""
  }${secs || ""}${secs ? ` second${secs == 1 ? "" : "s"}` : ""}`;
};

const ProcessMessageData = function (
  obj: JSONCompliantObject
...
let channels: {
  Static: {
    targetGuild: Discord.Guild | null;
    category: Discord.CategoryChannel | null;
    archive: Discord.CategoryChannel | null;
    logchannel?: Discord.TextBasedChannel;
  };
  ...
  global_cmds: [
    {
      ...
      action: (message, args) => {
        ...
        let createPageEmbed = function (page: number) {
          let f: string[] = targetTable
            .slice(5 * page, 5 * (page + 1))
            .map(
              (v) =>
                `**${v.names[0]}** ${v.names.slice(1).join(", ")}\n${v.desc}`
            );
        ...
        message.channel
          ...
          .then((msg) => {
            let col = msg.createMessageComponentCollector({
              ...
              idle: 30000,
              filter: (e) => {
                return e.user.id == message.author.id;
              },
            });
            ...
            col.on("collect", (int) => {
              ...
              pageNumber = clamp(
                0,
                Math.ceil(targetTable.length / 5) - 1,
                pageNumber + (int.customId == "ignore.helpRight" ? 1 : -1)
              );
                ...
                components: [
                  new Discord.MessageActionRow().addComponents(
                    ...
                    new Discord.MessageButton()
                      ...
                      .setCustomId("ignore.helpRight")
                      .setDisabled(
                        pageNumber == Math.ceil(targetTable.length / 5) - 1
                      )
                  ),
                ],
              });
        ...
        message.reply({
          embeds: [
            new Discord.MessageEmbed()
              .setThumbnail(
                "https://github.com/nbitzz/rocontrol/blob/dev/assets/rocontrol-app-icon.png?raw=true"
              )
              .setColor(_flags.BotDefaultEmbedColor)
              .setAuthor({ name: `RoControl ${_RoControl.version_int_name}` })
              .setTitle(`About RoControl`)
              .setDescription(
                `[RoControl](https://github.com/nbitzz/rocontrol) ${
                  _RoControl.version
                } (${_RoControl.state})\nMade with ${
                  emojis[Math.floor(Math.random() * emojis.length)]
                } by @nbitzz and other contributors\n[Update Tracker](https://github.com/users/nbitzz/projects/2/views/2)`
              )
              .addFields(
                {
                  name: "Contributors",
                ...
                {
                  name: "Uptime",
                  value: _TimeFormat(Math.floor(process.uptime())),
                  inline: true,
                }
              ),
          ],
    ...
    // Code for this command kinda sucks
    {
      ...
      action: (message, args) => {
        ...
        let _pr_read: (file: string) => Promise<Buffer | undefined> = (
          file: string
        ) =>
          new Promise((res, rej) => {
        ...
        fs.readdir(__dirname + "/../guides/meta", async (err, fnames) => {
          ...
          for (let [x, v] of Object.entries(fnames)) {
            let meta = (
              await _pr_read(__dirname + "/../guides/meta/" + v)
            )?.toString();

            if (meta) {
              let meta2 = JSON.parse(meta);
              meta2.article =
                __dirname + "/../guides/guide/" + v.split(".")[0] + ".md";
              metadata.push(meta2);
  ...
  local_cmds: [
          ...
          let f: string[] = targetTable
            ...
            .map((v) => {
              ...
              if (hasRoleId(v) && v.roleid) {
                displayNoEntrySign =
                  !message.member?.roles.cache.has(v.roleid) || false;
              }

              return `**${displayNoEntrySign ? "🚫 " : ""}${
                v.names[0]
              }** ${v.names.slice(1).join(", ")}\n${v.desc}`;
            });
    ...
    },
    {
      ...
      action: (session, message, args) => {
        message.reply(
          "This command will be released in a later version of 1.1 Quark. Thank you for your support!"
        );
      },
      args: 1,
    },
...
let OptipostServer = new Optipost(
  _flags.Port || 3000,
  "rocontrol",
  _flags.MaximumPayload || "100kb"
);

if (
  typeof _flags.StaticFileDirectory == "string" &&
  _flags.StaticFileDirectory
) {
  OptipostServer.app.use(express.static(_flags.StaticFileDirectory));
}
...
let OptipostActions: {
  [key: string]: (
    ...
    addLog: (lg: string, ts?: boolean) => void
  ) => void;
} = {
  GetGameInfo: (
    session: OptipostSession,
    ...
    channels.Dynamic[session.id].setName(
      (channels.other[session.id].limited ? _flags.LimitedModeLabel : "") +
        (data.data || "studio-game-" + session.id)
    );
    ...
    let ConnectionDialogueEmbed = new Discord.MessageEmbed()
      .setTitle("Connected")
      .setDescription(
        `Optipost Session ${session.id}\n\nJobId ${data.data}\nGameId ${data.gameid}\nPrivate Server Id ${data._PS_ID}\nPrivate Server Owner Id ${data._PS_UID}`
      )
      .setColor(
        _flags.RobustConnectionDialogueColor || _flags.BotDefaultEmbedColor
      );
    ...
    if (_flags.RobustConnectionDialogue) {
      axios
        .get(
          `https://www.roblox.com/places/api-get-details?assetId=${data.gameid}`
        )
        .then((datax) => {
          let up = datax.data.TotalUpVotes,
            down = datax.data.TotalDownVotes;
          ...
          ConnectionDialogueEmbed.setTitle(datax.data.Name)
            ...
            .setAuthor({
              name: datax.data.Builder,
              url: datax.data.BuilderAbsoluteUrl,
            })
            .addFields(
              {
              ...
              },
              {
                ...
                value: `${datax.data.VisitedCount} visits\n👍 ${
                  ...
                  Math.round((up / maxScore) * 10)
                )}${"⬛".repeat(
                  10 - Math.round((up / maxScore) * 10)
                )} ${Math.round((up / maxScore) * 100)}%`,
                inline: _flags.RobustConnectionDialogueFieldsAreInline,
              },
              {
                name: "Player Stats",
          ...
          if (_flags.RobustConnectionDialogueHasThumbnail) {
            axios
              .get(
                `https://thumbnails.roblox.com/v1/assets?assetIds=${data.gameid}&size=384x216&format=Png&isCircular=false`
              )
              .then((dataxx) => {
                if (dataxx.data.data) {
                  if (_flags.UseLargeImageForRobustConnectionDialogue) {
                    ConnectionDialogueEmbed.setImage(
                      dataxx.data.data[0].imageUrl
                    );
                  } else {
                    ConnectionDialogueEmbed.setThumbnail(
                      dataxx.data.data[0].imageUrl
                    );
                  }
                }
  ...
  Chat: (session: OptipostSession, data: JSONCompliantObject, addLog) => {
      ...
      if (channels.chnl_webhooks[session.id]) {
        ...
        channels.Dynamic[session.id].send({
          embeds: [
            new Discord.MessageEmbed()
              .setAuthor({
                name: _flags.UseCustomChatMessageUsername
                  ? _FlagFormat(_flags.ChatMessageUsernameLayout, {
                      DisplayName: data.displayname?.toString() || "?",
                      Username: data.username?.toString() || "?",
                      UserId: data.userid.toString(),
                ...
                iconURL: channels.imgcache[data.userid.toString()],
              })
    ...
    if (!channels.imgcache[data.userid.toString()]) {
      axios
        .get(
          `https://thumbnails.roblox.com/v1/users/avatar-headshot?userIds=${data.userid}&size=48x48&format=Png&isCircular=false`
        )
        .then((dt) => {
          if (!data.userid) {
            return;
  ...
  RegisterCommand: (
    ...
    if (
      ...
      typeof data.args_amt != "number"
    ) {
      return;
    }

    addLog(
      `Session registered command: ${data.id} (${data.names.join(
        ","
      )}), locked to role ${data.roleid}`
    );
      ...
      .then((user) => {
        ...
        let failSendMessage = () => {
          ...
          channels.Dynamic[session.id]
            .send(`A direct message to <@${user.id}> failed:`)
            .catch(() => {});
          channels.Dynamic[session.id]
            .send(ProcessMessageData(data.data))
            .catch(() => {});
        };

        if (
          channels.other._ratelimits.DMsThisMinute <=
            _flags.MaximumDirectMessagesPerMinute &&
          channels.other._ratelimits.DMsThisSecond <=
            _flags.MaximumDirectMessagesPerSecond
        ) {
          user
            .send(ProcessMessageData(data.data))
            .then(() => {
  ...
  ViaWebhook: (session: OptipostSession, data: JSONCompliantObject, addLog) => {
    ...
    axios
      .post(channels.chnl_webhooks[session.id].url, data.data)
      .catch(() => {});
  },
  SendMessage: (
    session: OptipostSession,
    ...
    if (data.data.replyto && typeof data.data.replyto == "string") {
      channel.messages.fetch(data.data.replyto).then((msg) => {
        if (msg) {
          ...
          msg
            .reply(ProcessMessageData(data.data))
            .then((msg) => {
              session.Send({
                type: "MessageSent",
                data: msg.id,
  ...
  DeleteMessage: (
    ...
    channels.Dynamic[session.id].messages
      .fetch(data.data)
      .then((msg) => {
        msg.delete();
      })
  ...
  GetInformationForMember: (
    ...
    if (channels.Static.targetGuild && typeof data.id == "string") {
      channels.Static.targetGuild.members
        .fetch(data.id)
        .then((memb) => {
          session.Send({
            type: "GuildMemberInformation",
            data: {
              validUser: true,
              tag: memb.user.tag,
              username: memb.user.username,
              discriminator: memb.user.discriminator,
              roles: Array.from(memb.roles.cache.values()).map((e) => {
                return e.id;
              }),
  ...
  ProcessImage: (
    ...
    axios
      ...
      .then((data) => {
        if (
          data.headers["content-type"].startsWith("image/") &&
          data.headers["content-type"] != "image/webp"
        ) {
          jimp
            .read(url)
            .then((img) => {
    ...
    axios
      ...
      .then((dt) => {
        session.Send({
          type: "GotHttp",
          key: data.key,
          data: { data: dt.data, headers: dt.headers, error: false },
        });
      })
      .catch((err) => {
  ...
  GetDiscordToRobloxChatEnabled: (
    ...
    session.Send({
      ...
      data: channels.other[session.id].DTRChatEnabled || false,
      key: data.key,
    });
  },
  SetDiscordToRobloxChatEnabled: (
    session: OptipostSession,
    data: JSONCompliantObject,
    addLog
  ) => {
  ...
  GetFeatures: (
    ...
    session.Send({
      type: "GotFeatures",
      data: Object.keys(OptipostActions).filter(
        (e) => data.all || !_config["api-disable"].find((a: string) => a == e)
      ),
      key: data.key,
    });
  },
...
OptipostServer.connection.then((Session: OptipostSession) => {
  ...
  let addLog = (str: string, addTs?: boolean) => {
    let dt = new Date();
    logs.push(
      `${!addTs ? dt.toLocaleTimeString("en-GB", { timeZone: "UTC" }) : ""} ${
        !addTs ? "|" : ""
      } ${str}`
    );
  };
    ...
    .then((channel: Discord.TextChannel) => {
          ...
          channel
            .send({
              embeds: [
                new Discord.MessageEmbed()
                  ...
                  .setDescription(
                    `The bot was unable to create a webhook within ${_TimeFormat(
                      _flags.LimitedModeNotificationTimer
            ...
            .then((msg) => {
              ...
              col.on("collect", (int) => {
                ...
                let memb = channels.Static.targetGuild?.members.resolve(
                  int.user
                );
      ...
      channel
        ...
        .then((webhook) => {
          ...
          let channels_until_archive_full =
            50 -
            Array.from(channels.Static.archive?.children?.values() || [])
              .length;

          if (
            channels_until_archive_full <= _flags.ChannelArchiveWarningLimit
          ) {
            channel.send({
              embeds: [
                new Discord.MessageEmbed()
    ...
    make_glot_post(logs.join("\n")).then((url: string) => {
      ...
      channels.Dynamic[Session.id]
        .send({
          embeds: [
            new Discord.MessageEmbed()
              ...
              .setDescription(
                `This channel will be automatically deleted in ${_TimeFormat(
                  _flags.ChannelAutoDeleteTimer
                )}. Click the Archive button to move it to the Archive category.`
              ),
          ],
          components: [
            new Discord.MessageActionRow().addComponents(
        ...
        .then((msg: Discord.Message) => {
          let col = msg.createMessageComponentCollector({
            componentType: "BUTTON",
            time: _flags.ChannelAutoDeleteTimer * 1000,
          });
...
client.on("ready", () => {
  ...
  client.user?.setPresence({
    activities: [
      {
        name:
          _FlagFormat(_flags.BotStatus || "", { prefix: prefix }) ||
          `${prefix}help | RoControl`,
        type: "PLAYING",
      },
    ],
  });
  ...
  client.guilds
    .fetch(_config.targetGuild.toString())
    .then((guild) => {
    ...
    if (Object.values(channels.Dynamic).find((e) => e == message.channel)) {
      ...
      for (let [x, v] of Object.entries(channels.Dynamic)) {
        if (v == message.channel) {
          ...
          let ltscmd = channels.local_cmds.find((e) =>
            e.names.find((a) => a == cmd)
          );
          ...
          if (ltscmd) {
            ...
            channels.logs[foundSession.id](
              `${message.author.tag} executed a TS command: ${message.content}`
            );
            ...
            try {
              ...
              message.reply(
                `An error occured while running this command. Please try again.`
              );
            }
          } else {
            // if not, try to find a lua cmd

            let lcmd = channels.cmdl[foundSession.id].find((e) =>
              e.names.find((a) => a == cmd)
            );
            if (lcmd) {
              ...
              channels.logs[foundSession.id](
                `${message.author.tag} ExecuteCommand: ${lcmd.id} (${message.content})`
              );
              if (lcmd.roleid) {
                  ...
                  message.channel.send({
                    embeds: [
                      new Discord.MessageEmbed()
                        ...
                        .setDescription(
                          `You do not have permission to run this command (${lcmd.id}).\n\nPlease obtain the role <@&${lcmd.roleid}>, then try again.`
                        ),
                    ],
                  });
      ...
      let globalCmd = channels.global_cmds.find((e) =>
        e.names.find((a) => a == cmd)
      );
        ...
        if (v == message.channel) {
          ...
          if (message.content && !Array.from(message.attachments.values())[0]) {
            foundSession.Send({
              ...
              tagColor: _flags.AutoTagColorization
                ? message.member?.displayHexColor || "ffefcd"
                : "ffffff",
              userId: message.author.id,
              messageId: message.id,
            });
          ...
          if (Array.from(message.attachments.values())[0]) {
            if (_flags.DisableImageSending) {
              message.reply(
                "Your administrator has disabled sending of images. If you would like to request that this ability be reinstated, please contact the owner of the RoControl server."
              );
              return;
            }
            channels.logs[foundSession.id](
              `${message.author.tag} uploaded an image: ${
                Array.from(message.attachments.values())[0].proxyURL
              }`
            );
                  ...
                  if (foundSession) {
                    ...
                    jimp.read(att.proxyURL).then((img) => {
                      ...
                      foundSession.Send({
                        ...
                        data: dtt,
                        caption:
                          cap.toLowerCase() == "none" || cap == ""
                            ? undefined
                            : cap,
                        time: parseInt(cset.time, 10) || 5,
                        player: cset.player || cset.target,
                      });
                    });
                  }
                }
    ...
    switch (int.customId) {
      case "Autoarchive":
        ...
        if (int.channel && int.message.id) {
          int.channel.messages
            .fetch(int.message.id)
            .then((msg) => {
              for (let [x, v] of Object.entries(channels.Dynamic)) {
                if (v.id == int.channelId) {
                  channels.other[x].autoarchive =
                    !channels.other[x].autoarchive;
      ...
      default:
        if (int.customId.startsWith("rcBtn.")) {
          ...
          for (let [x, v] of Object.entries(channels.Dynamic)) {
            if (v.id == int?.channel?.id) {
              let foundSession = OptipostServer._connections.find(
                (e) => e.id == x
              );
              if (!foundSession) {
                return;
...
if (_flags.DirectMessageRatelimit) {
  setInterval(() => {
    channels.other._ratelimits.DMsThisSecond = 0;
  }, 1000);
  setInterval(() => {
    channels.other._ratelimits.DMsThisMinute = 0;
  }, 60000);
}
